## Welcome to my **GitHub** home page~

<p align="center">  
  <img src="https://img.shields.io/badge/-JavaScript-e5cd0c?style=flat-square&logo=JavaScript&labelColor=f7df1e&logoColor=000" />  
  <img src="https://img.shields.io/badge/-TypeScript-blue?style=flat-square&logo=TypeScript&labelColor=CCEEFF&logoColor=blue" />  
  <img src="https://img.shields.io/badge/-Python-e5cd0c?style=flat-square&logo=Python&labelColor=f7df1e&logoColor=000" />  
  <img src="https://img.shields.io/badge/-HTML5-e34f26?style=flat-square&logo=HTML5&logoColor=fff" />  
  <img src="https://img.shields.io/badge/-Mathematica-cc0000?style=flat-square&logo=Wolfram&logoColor=white" />  
  <img src="https://img.shields.io/badge/-MATLAB-0076A8?style=flat-square&logo=MathWorks&logoColor=white" />  
  <img src="https://img.shields.io/badge/-R-276DC3?style=flat-square&logo=R&logoColor=white" />  
  <img src="https://img.shields.io/badge/-SPSS-006699?style=flat-square&logo=IBM&logoColor=white" />  
  <img src="https://img.shields.io/badge/-SAS-0071C5?style=flat-square&logo=SAS&logoColor=white" />  
  <a href="https://wakatime.com/@af33183b-1f14-4919-b7f7-17da9ae5e142">  
    <img src="https://wakatime.com/badge/user/af33183b-1f14-4919-b7f7-17da9ae5e142.svg" alt="wakatime" />  
  </a>  
</p>

<div align="center">
  <img src="https://skillicons.dev/icons?i=c,cpp,php,python,r,js,ts,html,css,less,latex,md,vue,react,nodejs,pytorch,webpack,mysql,idea,git,github,vscode,matlab,npm,ps,pr,linux,notion,postman,pycharm" />
</div>

<p align="center">👉 <a href="https://redhat123456.github.io/router_index/">Go to Portal 😋</a> 👈</p>

🎓 大家好！我是**数学专业的硕士生**，目前就读于桂林电子科技大学（GUET）。 
📌 我的研究兴趣包括：`PINN`、`谱方法`、`科学计算`和`深度学习`。  
🚀 我热衷于编程、探索和分享——梦想着发表论文（哪怕是“水论文”），同时在研究和工程实践方面脚踏实地。

📝 Blog: `https://redhat123456.github.io/`  
📮 Contact me:  
- Submit an [issue](https://github.com/redhat123456/redhat123456/issues/new)  
- Or email me at `1907065810@qq.com`

### 🎯 主要代码项目与贡献

+ **微信小程序** - [upPhysicalExercise](https://github.com/redhat123456/upPhysicalExercise)：集运动健身、视频播放与计划制订于一体的跨平台小程序，支持打卡、提醒及课程推荐。 ![](https://img.shields.io/github/stars/redhat123456/upPhysicalExercise?style=flat-square)
  - [Tanger_query](https://github.com/redhat123456/Tanger_query)：界面清爽的多功能查询小程序，涵盖天气预报、快递物流追踪等实用功能。 ![](https://img.shields.io/github/stars/redhat123456/Tanger_query?style=flat-square)

+ **Hexo 主题** - [hexo-theme-MiHoYo](https://github.com/redhat123456/hexo-theme-MiHoYo)：仿米哈游官网风格的博客主题，含渐变背景与交互动画，支持分类导航与访客统计，适配二次元向博客。 ![](https://img.shields.io/github/stars/redhat123456/hexo-theme-MiHoYo?style=flat-square)

+ **前端页面** - [resume](https://github.com/redhat123456/resume)：现代化的响应式个人简历模板，支持中英双语切换及在线展示与 PDF 导出。 ![](https://img.shields.io/github/stars/redhat123456/resume?style=flat-square)

+ **系统集成项目** - [Maple](https://github.com/redhat123456/Maple)：轻量级在线 Markdown 编辑器，支持实时预览、数学公式渲染、主题切换与导出，适合科研写作。 ![](https://img.shields.io/github/stars/redhat123456/Maple?style=flat-square)
  - [第七研究所创新基地官网](https://github.com/seven-innovation-base/official-website)：官方网站系统，包含新闻公告、项目展示与成员主页，并配备后台内容管理功能。 ![](https://img.shields.io/github/stars/seven-innovation-base/official-website?style=flat-square)

### 🏂 主要科研项目与贡献

+ **计算数学** - [FEMATHS 课题组官网](https://github.com/FEMATHS/cm.femaths.space)：计算数学学习资源整合平台，涵盖组内论文阅读记录、物理信息神经网络（PINN）实战分享及 GitHub 资源导航，致力于构建科研协作社区。 ![](https://img.shields.io/github/stars/FEMATHS/cm.femaths.space?style=flat-square)

---

### 🚧 进行中的项目

- **PINN 与谱方法的深度融合研究** 探索将 PINN 与高精度谱方法（如傅里叶、切比雪夫等）结合，利用频域正交基提升空间导数精度与网络收敛速度，以增强复杂物理问题的求解能力。

- **基于 PINN 的 Navier-Stokes 方程建模与预测** 应用 PINN 求解不可压缩流体 N-S 方程。引入自适应非均匀采样（如 Sobol 序列）与物理先验约束，减少样本浪费并缓解训练震荡，旨在开发适用于湍流及边界层分析的泛化模型。

- **多尺度可解释物理神经网络开发** 构建多尺度嵌套网络结合不确定性估计，增强复杂动力系统建模能力。计划融合图神经网络（GNN）处理非规则网格数据，并显式引入边界与守恒约束提升物理可解释性。

- **科研笔记与教学资源开源分享** 梳理 PINN、数值方法与深度学习的交叉研究（涵盖经典方程求解、训练技巧与误差分析），形成系统化学习资源库，将以博客或 Wiki 形式在 FEMATHS 官网持续开源更新。

### 🍁 自我提醒

- 保持沟通的高效与清晰
- 专注学习，充分休息。（八小时做题，八小时种花，八小时睡觉。）
- 保持真诚与积极 —— 直面现实，绝不退缩

<p align="center">  
  <a href="https://redhat123456.github.io" target="_blank">Blog</a> •  
  <a href="https://twitter.com/Tanger77300402" target="_blank">Twitter</a> •  
  <a href="https://www.zhihu.com/people/lan-de-qi-ming-liao-5" target="_blank">Zhihu</a>  
</p>

## 📊 GitHub Stats

<div align="center">

<!-- Profile -->
<img src="https://img.shields.io/github/followers/redhat123456?style=flat-square&logo=github&label=Followers" />

<img src="https://img.shields.io/github/stars/redhat123456/hexo-theme-MiHoYo?style=flat-square&logo=github&label=MiHoYo%20Theme" />

<img src="https://img.shields.io/github/stars/redhat123456/Maple?style=flat-square&logo=github&label=Maple" />

<img src="https://img.shields.io/github/stars/FEMATHS/cm.femaths.space?style=flat-square&logo=github&label=FEMATHS" />

</div>


<div align="center">

<img src="https://img.shields.io/github/languages/top/redhat123456/Maple?style=flat-square" />

<img src="https://img.shields.io/github/last-commit/redhat123456/Maple?style=flat-square" />

<img src="https://img.shields.io/github/repo-size/redhat123456/Maple?style=flat-square" />

</div>






[📅 Wakatime codetime](https://gist.github.com/redhat123456/4ee62e60852b37982d499d809324675b)

<!-- START_WakaGIST -->
```text
Markdown....... 12h 31m ........................31.0%
TypeScript..... 2h 25m ..........................6.0%
Python......... 1h 31m ..........................3.8%
MATLAB......... 1h 12m ..........................3.0%
Other.......... 22h 44m ........................56.2%
```
<!-- END_WakaGIST -->
把这个改得简洁一点并使用中英文两版
